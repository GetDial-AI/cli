...
const inputSchema = {
  ...
  language: z
    ...
    .optional()
    .describe(
      "BCP-47 language tag for the call. Omit to auto-detect from the destination number's country (alongside en-US).",
    ),
  voiceGender: z
    .enum(["male", "female"])
    .optional()
  ...
  transferTo: z
    ...
    .optional()
    .describe(
      "Forward-to number, E.164: the agent waits for a real human (riding out hold/IVR) then cold-transfers the call here. Must differ from `to` and the from number.",
    ),
  idempotencyKey: z
    .string()
    .optional()
    .describe(
      "Unique key (e.g. a UUID) making the placement idempotent: retrying with the same key returns the already-placed call instead of dialing again",
    ),
  fromNumber: z
    .string()
    .min(1)
