...
const inputSchema = {
  numberId: z
    ...
    .optional()
    .describe(
      "Filter to the contacts of a single phone number id on the account; counts and the latest " +
        "interaction are then that line's alone",
    ),
  limit: z
    .number()
    .int()
...
export const listContactsTool: ToolModule = {
  ...
  config: {
    ...
    description:
      ...
      "your numbers is one contact — pass `numberId` to see one line's contacts and counts instead. Group " +
      "conversations are not contacts; use list_groups for those. Pass a contact's `number` to list_messages " +
      "as `contact` to read that one conversation.",
    inputSchema,
