...
- **Connect WhatsApp to a number you already hold** — `dial number whatsapp <id|E.164|nickname>`, or buy a line with it using `dial number purchase --include-imessage --whatsapp`. Both are asynchronous: poll `dial number list` until the number's `whatsapp` status is `ready`. Beta — without access both answer `404` ([whatsapp.md](https://docs.getdial.ai/documentation/capabilities/whatsapp.md))
- **Wait for a new number to become usable** — `dial wait-for number.status_changed -f setupStatus=ready -f phoneNumberId=<id>`. Standard numbers are ready as soon as they're provisioned, so this returns straight away for them; an iMessage number takes a few minutes and can't send or call until `ready`. A setup that fails arrives on the same event with `setupStatus=failed` and a plain-language `setupError` — so wait on the event rather than on a timeout, and report the `setupError` to the user. This is the **number's own** setup, not the WhatsApp channel's: connecting WhatsApp is a separate track with its own `whatsapp` status (above), and it does not emit this event ([stream-account-events.md](https://docs.getdial.ai/documentation/capabilities/stream-account-events.md))
