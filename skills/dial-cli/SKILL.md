...
- **Set a number's inbound behavior** — `dial number set +14155550123 --inbound-instruction "..."`. The inbound instruction is the system prompt the AI uses on calls *into* that number; set it at `dial onboard` / `dial number purchase` time and change it here ([manage-phone-numbers.md](https://docs.getdial.ai/documentation/capabilities/manage-phone-numbers.md))
