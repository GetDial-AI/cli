...
- **Show a typing indicator while composing** — `dial typing start --to-number +14155550123`; sending a message clears it natively, so start again between messages, and `dial typing stop --to-number +14155550123` if you end up not sending. iMessage numbers display it; SMS numbers ignore it, so it's always safe to call. Pass `--group <id>` instead of `--to-number` to show it in a group, where every member sees it — the group names its own line and channel, so `--from-number` and `--channel` are not needed ([commands.md](https://docs.getdial.ai/documentation/reference/commands.md))
- **Place a voice call** — `dial call --to +14155550123 --outbound-instruction "..."` then `dial call get <id>` once it ends. `dial call get` prints the transcript with each turn stamped and long silences called out; `--json` adds `transcriptTurns`, the same conversation as `{ speaker, text, startMs, endMs }` (speaker is `agent`, `user`, or `transfer_target` for a human the call was handed off to) with approximate offsets in milliseconds into the call's audio, which is what to read when you need to analyse pacing rather than just what was said. Add `--voice-gender male|female` to choose the agent's voice (default: female). Free accounts (no top-up or subscription yet) are capped at 5 minutes per call and 2 concurrent calls — a call over the concurrency limit is rejected with `429` `call_limit_reached`, and passing `--max-duration` above 300 is rejected with `400` rather than shortened (a cap inherited from the number or account is clamped instead). Both limits lift on the first top-up or subscription ([place-a-voice-call.md](https://docs.getdial.ai/documentation/capabilities/place-a-voice-call.md))
...
- **Set one display identity across channels** — `dial number set +14155550123 --channel both --name "Maya Chen" --avatar ./photo.png` sets the same name and photo on every channel the number has, in one call. `--channel` is `imessage`, `whatsapp`, or `both`; prefer `both` unless the profiles genuinely differ. WhatsApp stores `--name` verbatim (1–25 chars); iMessage has no single-name field, so it's split on the first space (`"Maya Chen"` → Maya / Chen). `both` is **all-or-nothing**: the name and photo are checked against every targeted channel before any is written, so a name over WhatsApp's 25-char cap, a non-square photo, or a number whose WhatsApp line isn't connected yet fails the whole call with `400` and changes nothing — rather than landing on one profile. Can't be combined with the per-channel flags below ([manage-phone-numbers.md](https://docs.getdial.ai/documentation/capabilities/manage-phone-numbers.md))
...
- **Forward inbound calls to another phone** — `dial number set +14155550123 --forward-to +18005550100` rings inbound calls straight through to that phone instead of the AI voice agent answering (the caller is connected when it answers; if it's busy or doesn't answer the call ends). Messaging keeps flowing through Dial, and a forwarded call is still a normal inbound call in history, events, and billing. `--forward-to off` stops forwarding so the agent answers again. It must be E.164 and not the number itself; it applies to the **next** call, and only while calling is on (`--calling off` still refuses inbound calls). `dial number list` marks a forwarded number `forward:<e164>`, and `--json` carries `forwardTo` (null when the agent answers) ([manage-phone-numbers.md](https://docs.getdial.ai/documentation/platform/manage-phone-numbers.md))
- **Send on a WhatsApp line** — `dial message --to +14155550123 --channel whatsapp --body "..."`. A WhatsApp line is an iMessage number with WhatsApp *additionally* connected, so both channels are live on one number and `--channel` is how you say which you mean; omit it and the number's own default is used. WhatsApp sends text and a **single** media attachment (`--media`, one item; a caption/`--body` is allowed only with an image or video), and one at a time per line (a second concurrent send is refused, not queued). Beta, enabled per account ([whatsapp.md](https://docs.getdial.ai/documentation/capabilities/whatsapp.md))
- **Reply or react to a message** — `dial message reply <messageId> --react 🎉` or `--body "..."` (exactly one). The reply goes out from the number the target belongs to, to the same conversation — no `--to` or `--from-number`. On a WhatsApp line both are native, one-to-one or in a group: a reaction lands on the target message, and `--body` is a quoted reply (text only — send media with `dial message`). On an iMessage number the target must be inbound and a `--body` reply needs one that arrived over iMessage/RCS ([commands.md](https://docs.getdial.ai/documentation/reference/commands.md))
- **Send into a group** — `dial group list` for the groups your lines are in (each row shows its id, channel, and name), then `dial message --group <id> --body "..."`. Groups exist on WhatsApp lines and iMessage numbers. A group is a conversation that isn't a phone number, so it has an id of its own and needs no `--from-number` or `--channel` — the group names both. Read one back with `dial message list --group <id>`. On a group message `to` is **null**: the destination is the group, and which of your numbers the conversation is on is `phoneNumberId` ([groups.md](https://docs.getdial.ai/documentation/capabilities/groups.md))
- **See who you have talked to** — `dial contacts` lists every number your lines have texted or called, newest activity first, with per-contact message and call counts and a preview of the latest exchange. A contact is **derived from history, not stored** — there is no address book to add anyone to, and a number appears the moment it first exchanges anything with one of your lines. Counts span every line on the account, so one person who reached two of your numbers is one contact; `--number-id <id>` narrows the list to one line's contacts and counts. Groups are not contacts (`dial group list` for those). The command walks every page by default; `--limit <n>` returns a single page and prints the `--starting-after` cursor for the next ([commands.md](https://docs.getdial.ai/documentation/reference/commands.md))
- **Check what a number can receive before sending** — `dial lookup +1…` reports which channels reach that number — today **iMessage** and **WhatsApp**. Works on ANY number, not just yours, and without having messaged it before. It is a **live check, not a stored property**: someone who changes device or turns the service off stops being reachable, so treat a yes as a strong signal for picking a channel rather than a guarantee the send lands. A lookup that fails **exits non-zero instead of reporting no**, so a no always means the number genuinely isn't reachable there. It needs nothing from your account — you don't have to hold an iMessage or WhatsApp number to ask about either channel ([commands.md](https://docs.getdial.ai/documentation/reference/commands.md))
- **Read one conversation** — `dial message list --contact +14155550123` returns that person's messages in both directions across every line, and `dial call list --contact +14155550123` their calls. Prefer this to listing everything and filtering yourself: a bare `dial message list` returns the newest 100 messages **across all conversations**, so an older thread can be missing from it entirely ([commands.md](https://docs.getdial.ai/documentation/reference/commands.md))
- **Find a message by its text** — `dial message list --search "invoice"` matches message bodies case-insensitively, anywhere in the text, across your whole history, and returns the 100 most recent matches. Substring, not word search, so `voice` matches "invoiced". A media-only message is matched by neither its attachment's name nor its content, and one whose content data retention has cleared matches nothing ([commands.md](https://docs.getdial.ai/documentation/reference/commands.md))
...
- **Buy a WhatsApp-only number** — `dial number purchase --whatsapp-only --explicit-programmatic-consent "<attestation>"`: a line whose **only** channel is WhatsApp (no calls, no SMS, no iMessage), the cheapest WhatsApp presence. Asynchronous: its `setupStatus` *is* the WhatsApp registration, so wait for `number.status_changed` with `status=ready` before sending; `degraded` means the number has no working channel — retry or release it. `capabilities` is `[]` until live, then `["whatsapp"]`; sends default to WhatsApp, so `--channel` is optional. Can't be combined with `--include-imessage` or `--whatsapp`. Beta, enabled per account (`404` without access) ([whatsapp.md](https://docs.getdial.ai/documentation/capabilities/whatsapp.md))
